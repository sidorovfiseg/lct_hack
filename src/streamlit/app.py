...
uploaded_file = st.file_uploader('Загрузите файл', type=['csv', 'xlsx'], help='Пожалуйста, загрузите файл в формате CSV.')

limit = st.number_input('Введите максимальное количество патентов каждой компании', min_value=10, value=1000, max_value=1000)
...
if st.button('Загрузить документ', disabled=uploaded_file is None):
    if uploaded_file is not None:
        ...
        if uploaded_file.name.endswith('.xlsx'):
            ...
            file_content = StringIO(data.to_csv(index=False))
        elif uploaded_file.name.endswith('.csv'):
            file_content = StringIO(uploaded_file.getvalue().decode("utf-8"))
        
        response = get_doc_markup(file_content, limit=limit)
        ...
        if response is not None and response.status_code == 200:
            ...
            if limit <= 100:
                ...
                if clusters_response:
                    ...
                    st.error('Ошибка при получении данных о кластерах')
            else:
                st.warning("Количество патентов больше 100, кластеры не будут загружены")
